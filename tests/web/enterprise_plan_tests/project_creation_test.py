...
def test_new_project_creation(app: Application, login):
    ...
    (app.new_projects_page.open().is_loaded().fill_project_till(target_project_name).click_create())

    (app.project_page.is_loaded().empty_project_name_is(target_project_name).close_read_me())
...
def test_new_project_creation_and_test_popup(logged_app: Application):
    ...
    (logged_app.new_projects_page.open().is_loaded().fill_project_till(target_project_name).click_create())

    project_page = logged_app.project_page
    (project_page.is_loaded().empty_project_name_is(target_project_name).close_read_me())

    (project_page.side_bar.is_loaded().click_logo().is_tab_active("Tests"))
...
def test_open_project_and_create_test_suite_from_side_bar(api_client: ApiClient, logged_app: Application):
    ...
    target_project = next(p for p in all_projects["data"] if p["attributes"]["title"] == "Grocery, Outdoors & Shoes")
